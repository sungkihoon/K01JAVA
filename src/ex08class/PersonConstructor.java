...
/*
 ...
 - 반환값이 없다. 즉 특정 연산 후 값을 반환할 수 있다. 따라서 void를 쓸 수 없다.
- 그 외는 메소드가 가진 모든 특성을 가지고 있다.(매개변수 전달, 오버로딩 등)
 ...
 용도1 : 동일 클래스 내에서 다른 생성자를 호출할 때 사용한다.
 	...
 	사용법 :
 		this(인자1,인자2,....);
 용도2 : 생성자 내에서 멤버변수와 매개변수의 이름이 동일할 때 구분하기 위해 사용한다.
 	this.변수 => 멤버변수
 	변수 => 생성자의 매개변수
...
public class PersonConstructor {
	...
	/*
	 디폴트 생성자
	 	: 해당 클래스에 생성자를 정의하지 않은 경우에 아래와 같은 매개변수, 실행부가 없는 생성자가
	 	자동으로 삽입된다. 단, 생성자를 하나라도 정의하면 디폴트 생성자는 삽입되지 않는다.(주의)
	 */
	...
	//멤버 메소드 : 멤버변수를 초기화 할 목적으로 정의한 메소드
