...
/*
 ArrayList :List계열의 컬렉션으로
 		- 데이터의 중복저장이 허용된다
 		- 데이터의 저장순서를 보장한다
 		- 데이터에 접근할때 iterator 혹은 get을 이용한다
	...
	public static void main(String[] args) {
		/*
		 List<E> 인터페이스 계열의 컬렉션
		 	...
		 	아래와같이 <T>부분을 생략하면 모든 객체를 저장할 수있는 컬랙션이된다
		...
		list.add("트와이스");
		list.add(3,"워너원");
		//list.add(5,"소방차 ㅋㅋ");//애러발생. 인덱스는 건너뛸수 없다
		list.add(list.size(),"오마이걸");// <- 4번 인덱스에 입력됨
		list.add(list.size(),"방탄소년단");
		System.out.println("중복된 저장된 객체수: "+list.size());
		...
		/*
		 1-4] 덮어쓰기
		 	set(인덱스, 객체) : 특정 인덱스에 저장된 데이터를 기본 객체에 저장
		 */
		...
		/*
		 1-6] 포함여부 확인
		 	contains(객체명) : 찾고자 하는 객체가 있는경우 true반환
		 */
		...
		/*
		 ...
		 방법1 : 인덱스로 삭제
		 	remove(인덱스) : 인덱스를 통해서 삭제를 진행한후
		 				삭제가완료되면 해당 객체를 반환한다
		 				삭제후에는 인덱스가 자동으로 재부여 된다
		 */
		Object obj =list.remove(2);
		System.out.println("삭제된 객체:"+obj);
		
		//방법2 : indexOf()메소드로 인덱스로 찾은후 삭제
