...
//		map.put("age", number); //[에러발생] value타입이 맞지않음
		...
		/*
		 4. 중복저장
		 	: 기존에 입력된 name이라는 key값이 있으므로 "홍길동"이 출력된다.
		...
		/*
		 5-2]키값을 모를때 출력하기
		 	...
		 	3. 키값을 얻어온 후 확장for문을 통해 key값을 하나씩 출력한다.
		 */
		...
		/*
		 5-3]반복자를 통한 출력
		 	: iterator을 통해 반복할때도 keySet()으로 키값을 먼저
		 	얻어온 후 사용할 수 있다.
		 */
		...
		while(it.hasNext()) {
			String key = it.next(); //키값 남자
			String value = map.get(key); //value얻기
			System.out.println(String.format("%s %s", key, value));
		}
		...
		/*
		 7]삭제
		  	: key값을 통해 삭제하고, 삭제가 완료되면
		  	삭제된 key값에 해당하는 value가 반환된다.
		 */
