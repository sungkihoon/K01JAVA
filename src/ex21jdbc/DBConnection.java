...
public class DBConnection {
	...
	public static void main(String[] args)
		...
		if(con!=null) {
			...
			/*
			 3] 쿼리문작성
		 			: 쿼리문을 작성할때 주의할 점은 줄바꿈을 할때 앞뒤로 스페이스를
		 			삽입하는 것이 좋다. 그렇지 않으면 서로 문장이 이어지게 되어
		 			SyntaxError가 발생하게 된다.
			 */
			String sql = "select * from employees where "
					+ "department_id=50 order by employee_id desc";
			
			/*
			 4] 쿼리문 전승을 위한 Statement계열의 객체를 생성한다.
			 	해당 객체는 주로 정적인 쿼리를 실행할때 사용된다.
			 */
			Statement stmt = con.createStatement();
			
			/*
			 5] Statement객체를 사용하여 오라클로 작성된 쿼리문을 전승한다.
			 	오라클은 전승받은 쿼리를 실행한 후 결과를 반환하는데
			...
			while(rs.next()) {
				...
				/*
				 오라클이 반환해준 ResultSet객체의 갯수만큼 반환하면서
