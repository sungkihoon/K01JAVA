...
/*
 친구를 포함한 최상위 클래스로 해당 프로그램에서는 Friend로 객체생성은 하지 않는다.
 */
class Friend{
	String name;
	String phone;
...
class FriendInfoHandler{
	...
	/*
	 ...
	 하나의 배열 내에 2개의 객체를 동시에 저장하여 관리할 수 있다.
	 */
	...
	/*
	 ...
	 3. 객체배열의 저장된 객체들을 for문을 통해 순차적으로 출력. 이때 오버라이딩된 메소드를 통해
	  	항상 해당 객체의 정보를 출력할 수 있음.(참조변수의 타입에 영향을 받지 않음)
	 */
	...
	public void searchInfo() {
		...
		for(int i=0;i<numOfFriends;i++) {
			...
			if(searchName.compareTo(myFriends[i].name)==00) {
				myFriends[i].showAllData();
	...
	public void deleteInfo() {
		...
		for(int i=0;i<numOfFriends;i++) {
			if(deleteName.compareTo(myFriends[i].name)==00) {
