...
/*
 구성관계로 표현하기에 적합한 Has-A관계
 	...
 	Has-A와 같이 관계를 상속으로 표현하면 클래스간의 강한 연결고리가 형성되어
 	권총이 없는 경찰을 표현하기가 힘들어진다. 따라서 이 경우에는 구성관계로 표현하는 것이
 	적합하다. 구성관계는 클래스의 멤버로 또 다른 객체를 정의하는 것을 말한다. 
 */
class Gun{
	int bullet;//총알의 갯수
	public Gun(int bNum) {
...
class Police{
	...
	public Police(int bNum, int hCuff) {
		...
		/*
		 권총 보유여부 초기화
		 	: 만약 총알이 없는 상태라면 보유한 권총이 없는 것으로 표현하기 위해
		 	참조별수를 null로 초기화한다. 참조변수가 null값을 가진다는 것은
		 	아무것도 참조할 객체가 없다는 의미이다.
		 */
		if(bNum<=0) {
			gun = null;
		}
