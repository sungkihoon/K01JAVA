...
/*
 Is-A관계
 	...
 	- 노트북은 컴퓨터의 일종이다.
 	- 휴대폰은 전화기의 일종이다.
 	이와같이 Is-A관계가 성립하는 기체를 상속으로 표현하기에 적합하다.
 */
...
class Computer{
	...
	/*
	 private 접근지정자로 선언된 멤버메소드는 외부클래스 혹은 상속받은 클래스에서
	  접근이 불가능하므로 public으로 선언된 메소드를 통해서 간접호출을 해야한다.
...
class NotebookComputer extends Computer{
	...
	public void movigCal() {
		...
		/*
		 아래 메소드는 private으로 선언되었으므로 상속받은 자식클래스에서도 접근은 불가능
		 */
		//		keyboardTouch();
		//		calculate();
...
class TabletNotebook extends NotebookComputer{
	...
	//태블릿 사용을 위해 배터리양 체크 및 등록된 펜이지 체크
...
public class E04ISAInheritance {
	...
	public static void main(String[] args) {
		...
		System.out.println("==ISE-1234 펜으로 테플릿 사용==");
