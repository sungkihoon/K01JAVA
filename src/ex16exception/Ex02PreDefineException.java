...
/*
 ...
 - RuntimeException 클래스 계열로 NullPointerException이
 	대표적인 하위 클래스이다.
 - main메소드에서 무시하기 위해 throws하더라도 오류가 발생되므로 반드시 예외처리를 해줘야 한다.
 - 예외발생시 JVM은 해당 예외클래스를 인스턴스화해서 프로그램쪽으로 전달하게 되고
 	try-catch문에서 받은 후 처리한다.
 */
public class Ex02PreDefineException {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 : 배열의 크기를 벗어난 인덱스를 사용할 때 발생되는 에러
		 */
		...
		/*
		 ...
		 : 숫자형식의 문자열을 int로 변환하는경우 해당 문자열이 숫자형식이 아니면 발생되는 예외.
		 */
		...
		try {
			System.out.print("나이를 입력하세요 : ");
			int intAge = sc.nextInt();//<=문자열을 입력하면 예외발생
		...
		/*
		 4] NullPointerException
		 	...
		 	즉 참조변수만 생성되고, 참조할 객체가 없는 상태에서 사용했을때 발생하게된다.
		 */
		System.out.println("### NullPointerException ###");
		System.out.println("toDay = "+toDay);/*
							toDay 참조변수가 가지고있는 null값을
		 ...
		 */
		System.out.println("### ArithmeticExceiption ###");
		int result = 10;
		try {
			result = result / 0;
